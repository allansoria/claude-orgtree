...
class GitWorkspaceTests(unittest.TestCase):
    ...
    @unittest.skipUnless(hasattr(Org, "work_create"),
                         "branch<->work-item links need upstream's work-items subsystem, which this tree has not ported. gitworkspace and gitapi read the items through a defaulted facts.get('work_items', []), so without it linking degrades to an empty offer list and refuses every slug rather than failing - the behaviour the other tests here cover. Port workitems.py and this test runs itself.")
