...
import { setOpenRouterTiers } from '../src/canvas/shared'
...
test('§2 one template row → worker_template, two → worker_templates[]', async () => {
  ...
  try {
    ...
    await planAndCreate(v.el, async () => {
      ...
      // upstream's antigravity tier; the fork's vocabulary had 'orbit'
      await pick(tiers[1], 'pro', 'tier 1')
    })
    b = calls.find((c) => c.path.endsWith('/queues'))!.body as Record<string, unknown>
    const cfg2 = b.config as Record<string, unknown>
    assert.deepEqual(cfg2.worker_templates, [{ tier: 'sonnet' }, { tier: 'pro' }],
...
// Re-aimed 2026-09-09. The fork's version filled a MODEL field and left the
// tier blank; there is no model field here (an OpenRouter favorite is itself a
// tier), so a CHARTER now plays that role: it is the other optional field that
// makes a row non-empty, and a non-empty row still owes a tier.
test('§3 a template row with a charter but no tier blocks create', async () => {
  useFakeClock()
  try {
    const calls = stubWithPlan({})
    const v = await mountPanel([], {})
    await planAndCreate(v.el, async () => {
      await click([...v.el.querySelectorAll('button')].find(
        (b) => b.textContent === '+ worker template'), 'add tpl')
      await type(v.el.querySelector('.queue-cfg-charter'), 'reduce carefully', 'charter')
    })
...
// §6 RETIRED 2026-09-09 (it covered "the model catalogue fills a template
// row"). The catalogue browser is gone from this panel, and deliberately: in
// the fork a worker row needed a tier (a fixed band) AND a separate OpenRouter
// model id, so it had a "browse…" button to fill the second field. Here an
// OpenRouter favorite IS a tier, so the tier dropdown already lists every
// favorite by model name and there is no second field to fill. The behaviour
// this test pinned — "a browse never clobbers a tier the user set" — cannot
// occur any more, because nothing writes a tier except the user.
//
// What replaces it is §7's group assertion plus §7b below, which pins the part
// that IS new: that the dropdown reads the favorites list at RENDER time.

test('§7b the tier dropdown reads the OpenRouter favorites AT RENDER', async () => {
  // The invariant that replaces the retired §6, and the reason `tierGroups()`
  // is a function rather than the module-level constant the fork used: an
  // OpenRouter favorite is minted at RUNTIME, so a list frozen at import would
  // never show one the user selected afterwards. Seed a favorite, then assert
  // the group and the option appear — and that the option is labelled with the
  // MODEL name, not the raw `or-…` tier id.
  useFakeClock()
  try {
    setOpenRouterTiers([{
      // the tier id must match shared.ts's OR_TIER_RE (/^or-[a-z0-9-]+$/):
      // no dots, so a model's `v3.2` becomes `v3-2` in the tier id
      tier: 'or-deepseek-v3-2', provider: 'openrouter', seat: 1,
      model: 'deepseek/deepseek-v3.2', letter: 'D', name: 'DeepSeek V3.2',
      label: 'DeepSeek V3.2', color: '#4b6bfb',
    }])
    stubWithPlan({})
    const v = await mountPanel([], {})
    await click([...v.el.querySelectorAll('button')].find(
    ...
    assert.ok(groups.includes('OpenRouter'),
      `a minted favorite must add its provider group — saw ${JSON.stringify(groups)}`)
    const opts = [...sel.querySelectorAll('option')]
    const or = opts.find((o) => o.value === 'or-deepseek-v3-2')
    assert.ok(or, 'the favorite is not offered as a tier')
    assert.equal(or.textContent, 'DeepSeek V3.2',
      'the option must read as the model, not the raw or- tier id')
    await v.unmount()
  } finally {
    setOpenRouterTiers([])          // the module list is global; do not leak
    realClock()
  }
})
...
test('§7 the tier field is a grouped dropdown, blank until chosen', async () => {
  ...
  try {
    ...
    const groups = [...sel.querySelectorAll('optgroup')].map((g) => g.label)
    // ⚠ NOT 'OpenRouter' any more (ported 2026-09-09). In the fork the
    // OpenRouter tiers were a FIXED vocabulary (spark…nova), so that group
    // was always present. Here an OpenRouter favorite IS a tier the user
    // mints at runtime, so with no favorites selected there is legitimately
    // no OpenRouter group to find — asserting one would pin behaviour this
    // tree does not have. The static providers are the real invariant; §7b
    // below covers the dynamic half.
    assert.ok(groups.includes('Claude') && groups.includes('Codex'),
      `tier options must be grouped by provider — saw ${JSON.stringify(groups)}`)
    const opts = [...sel.querySelectorAll('option')].map((o) => o.value)
    // upstream's static vocabulary. 'spark' (a fork OpenRouter band) and
    // 'orbit' (the fork's antigravity tier) do not exist here.
    for (const t of ['sonnet', 'opus', 'sol', 'flash', 'pro']) {
