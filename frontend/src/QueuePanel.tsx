...
import { ALL_TIERS, PROVIDER_LABEL, openrouterTierIds, providerOf, tierLabel }
  from './canvas/shared'
 ...
 *
 *  ⚠ COMPUTED PER CALL, not once at module load as the fork had it. An
 *  OpenRouter favorite here IS a tier (`or-<model id>`, D-232) and the user
 *  mints them at RUNTIME, so a module-level constant would freeze the list at
 *  first import and a favorite selected afterwards would never appear in this
 *  dropdown. `ALL_TIERS` carries only the static providers; the OpenRouter
 *  half has to be asked for every time. */
const tierGroups = (): ReadonlyArray<readonly [string, string[]]> => {
  const g: Record<string, string[]> = {}
  for (const t of [...ALL_TIERS, ...openrouterTierIds()]) {
    (g[providerOf(t)] ??= []).push(t)
  }
...
function TierSelect({ value, onChange, blankLabel, className }: {
  ...
  return (
    <select className={className} value={value}
      ...
      {tierGroups().map(([prov, tiers]) => (
        <optgroup key={prov} label={PROVIDER_LABEL[prov] ?? prov}>
          {/* an OpenRouter tier's id is `or-<model id>`; `tierLabel` is the one
              door that turns that into the model's display name */}
          {tiers.map((t) => (
...
interface TemplateRow { tier: string; charter: string }
interface DirRow { path: string; mode: 'rw' | 'ro' }

const emptyTemplate = (): TemplateRow => ({ tier: '', charter: '' })
...
function QueuePlanner({ slug, onCreated }: {
  ...
  const buildConfig = (): Record<string, unknown> | undefined => {
    ...
    const rows = templates
      .map((t) => ({
        tier: t.tier.trim(), charter: t.charter.trim(),
      }))
      .filter((t) => t.tier || t.charter)
            ...
            <div className="queue-cfg-row" key={i}>
              <TierSelect className="queue-cfg-tier" value={t.tier}
                ...
                onChange={(v) => setTemplates((rows) => rows.map(
                  (r, j) => j === i ? { ...r, tier: v } : r))} />
              {/* ⚠ NO model field, and no "browse…" button. In the fork a
                  worker row carried a tier (a fixed band: spark…nova) AND a
                  separate OpenRouter model id, so it needed a catalogue
                  browser to fill the second. Here an OpenRouter favorite IS a
                  tier (D-232), so the TierSelect above already lists every
                  favorite by model name and there is no second field to fill.
                  A model the user has not made a favorite is not hireable at
                  all — which is upstream's rule, not a limitation of this
                  panel. */}
    ...
    <details className="queue-panel">
      ...
      <div className="queue-panel-body">
        ...
        {rows.map((queue) => {
          ...
          // ⚠ this splits spend by PROVIDER, and the thing it asks about is a
          // TIER. In the fork a template carried a band plus a model id, so
          // this fell back to `.model` when `.tier` was absent; here the tier
          // is the whole identity (an OpenRouter favorite is `or-<model id>`)
          // and that fallback could never fire, so it is gone.
          const fallbackTier = queue.config.worker_template?.tier ?? ''
          let real = 0
          let quota = 0
          for (const [worker, spend] of Object.entries(queue.cost.by_worker)) {
            // OpenRouter is real dollars off a key; every other lane is
            // subscription quota. Only the split is shown, never a total.
            const tier = workerModels[worker] ?? fallbackTier
            if (providerOf(tier) === 'openrouter') real += spend
            else quota += spend
          }
          return (
                ...
                <div>
                  ...
                  {Object.keys(queue.cost.by_worker).length ? (
                    <ul>{Object.entries(queue.cost.by_worker).map(([worker, spend]) => {
                      // the tier IS the model identity here, and `tierLabel`
                      // renders an `or-<model id>` favorite as its model name
                      const tier = workerModels[worker] ?? fallbackTier
                      return <li key={worker}><b>{worker}</b> · {tier ? tierLabel(tier) : 'unknown'} · {dollars(spend)}</li>
                    })}</ul>
